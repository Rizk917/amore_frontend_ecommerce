...
function ProductsAdmin() {
  ...
  const editProduct = async (id) => {
    try {
      const url = `https://amore-backend.onrender.com/products/${id}`;
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post("https://amore-backend.onrender.com/products", formData, config)
      .then((response) => {
  ...
  const handleUpdateProduct = (event) => {
    ...
    axios
      .put(
        `https://amore-backend.onrender.com/products/${productsById._id}`,
        formData,
        config
      )
      .then(() => {
